...
import { CreateListingRequest, Condition, Gender } from '../../types/listing.types';
...
export const CreateListingPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const sneakerCheck = await catalogService.getSneakerBySku(formData.sneakerSku);
      ...
      if (response.success && response.data) {
        ...
        setMessage({ type: 'error', text: response.error?.message || 'Error al crear listing' });
      }
    } catch (err) {
  ...
  const handleSneakerCreated = async (sneakerData: CreateSneakerRequest) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        setMessage({ type: 'success', text: 'Sneaker y listing creados exitosamente' });
        setTimeout(() => {
          navigate('/seller/listings');
        }, 1500);
      } else {
        ...
        <Card className={styles.card}>
          ...
          <form onSubmit={handleSubmit} className={styles.form}>
            <SkuAutocomplete
              ...
              onChange={(sku) => setFormData((prev) => ({ ...prev, sneakerSku: sku }))}
