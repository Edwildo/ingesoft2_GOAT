...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const login = useCallback(async (
    ...
    try {
      const response: ApiResponse<{ token: string; email: string; roles: string[]; userId: string }> = 
        await authService.login({ email, password });
      
      if (response.success && response.data) {
        const { token: newToken, userId, email: userEmail, roles } = response.data;
  ...
  const register = useCallback(async (
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const errorMessage = response.error?.message || 'Error al registrar usuario';
        return { success: false, message: errorMessage };
      }
    } catch (error) {
      return { success: false, message: 'Error de conexión con el servidor' };
